...
class GameStateWatcher extends EventEmitter {
    constructor() {
        ...
        if (os.platform() == 'win32') {
            this.accountDir = pathResolver.join(os.homedir(),'','Documents','Heroes of the Storm','Accounts')
            this.battleLobbyDir = pathResolver.join(os.tmpdir(),'','Heroes of the Storm')
        } else if (os.platform() == 'darwin') {
            this.accountDir = pathResolver.join(os.homedir(),'','Library','Application Support','Blizzard','Heroes of the Storm','Accounts')
            this.battleLobbyDir = pathResolver.join(os.homedir(),'','Library','Caches','TemporaryItems','Blizzard','Heroes of the Storm')
        } else {
