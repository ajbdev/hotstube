...
class HighlightReel extends EventEmitter {
    ...
    getSavePath(file) {
        let dirName = pathResolver.basename(this.replayFile, '.StormReplay')
        let path = [app.getPath('videos') ,'HotS Tube', dirName].join(path.sep)

        if (!fs.existsSync(path)) {
            fs.mkdirSync(path)
        }

        return [path, file].join(path.sep)
    }
    ...
    create() {
        ...
        fights.map((fight) => {
            ...
            let fileName = this.getSavePath(this.player.name + ' - ' + time);
